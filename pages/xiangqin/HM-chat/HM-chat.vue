<template>
	<view>
		<view class="content" @touchstart="hideDrawer">
			...
			upper-threshold="50">
				<!-- 加载历史数据waitingUI -->
				<view class="loading" v-show="loading_show">
					<view class="spinner">
						...
						<view class="my" v-if="row.msg.userinfo.uid==myuid">
							...
							<view class="left">
								...
								<view v-if="row.msg.type=='text'" class="bubble">
									...
									<view class="" v-html="row.msg.content.text">
							...
							<!-- 右-头像 -->
							<view class="right">
								<image :src="row.msg.userinfo.face" mode="aspectFill" @click="go_details(row.msg.userinfo.uid)"></image>
							</view>
						</view>
						<!-- 别人发出的消息 -->
		...
		methods:{
			...
			go_details(n) {
					...
					if(n == 0){
						let uid = id
						uni.navigateTo({
							url: '/pages/xiangqin/user/userinfo?data=' + uid
						})
					}else if(n == 1){
			...
			//将消息状态变为已读
			readed(){
				this.$http.post('lessonxiangqin/api/lessonupdatexiaoxistatus',{
			...
			jieshou(){
				...
				uni.onSocketMessage(function (res) {
				  ...
				  // console.log(o)
				  let content =  _this.replaceEmoji(o.postmessages)
				  let duiId = _this.user_id
				  console.log(duiId)
				  console.log(o.fromuserid)
				  console.log(o.touserid)
				  if(o.fromuserid === duiId){
					  _this.msgList.push({
					  	type: 'user',
					  	msg: {
					  		id: o.id,
					  		type: 'text',
					  		// time: moment(o.time).format('YYYY-MM-DD HH:mm:ss'),
					  		time: o.times,
					  		userinfo: {
					  			uid: o.touserid === uni.getStorageSync('userId') ? 1 : 0,
					  			username: _this.user_name,
					  			face: o.fromuserphoto
					  		},
					  		content: {
					  			text:content
					  		}
			...
			//触发滑动到顶部(加载历史信息记录)
			loadHistory(e){
				if(this.is_more === false){
					return ;
				}
				this.loading_show = true
				this.scrollAnimation = false; // 关闭滑动动画
				let Viewid = this.msgList[0].msg.id; //记住第一条信息id
				console.log(Viewid)
				this.$http.post('lessonxiangqin/api/lessonltxiangqing', {
						userid : uni.getStorageSync('userId'),
					...
					.then(res => {
						let lists = _.reverse(res.data.rows)
						_.each(_.reverse(res.data.rows), o => {
							this.msgList.unshift({
								...
								msg: {
									...
									content: {
										text: this.replaceEmoji(o.postmessages)
									}
								}
							})
						})
			...
			getMsgList(){
				...
				let _this = this
				_this.$http
				.post('lessonxiangqin/api/lessonltxiangqing', 
				{
					userid : id,
					pageNum: _this.pageNum,
					pageSize: 15,
					fromuserid: _this.user_id
				}, {})
				.then(res => {
					...
					_.each(_.reverse(res.data.rows), o => {
						_this.msgList.push({
							...
							msg: {
								...
								time: o.times,
								userinfo: {
									uid: o.touserid == id ? 1 : 0,
									username: _this.user_name,
									// face: o.touserid == id ? o.touserphoto : o.fromuserphoto  
									face: o.fromuserphoto
								},
								content: {
									text: _this.replaceEmoji(o.postmessages)
								}
							}
						})
					})
					_this.more_page = Math.ceil(res.data.total/15)
					if (_this.pageNum === this.more_page) {
						_this.is_more = false
					}
					_this.pageNum++
					_this.$nextTick(function() {
						// 滚动到底
						_this.scrollToView = 'msg'+ this.msgList[this.msgList.length - 1].msg.id
					});
					console.log(_this.msgList)
				})
				.catch(err => {
			...
			sendText(){
				...
				if(payDeposit != 2 && this.liaotianrenshu >= 3){
					...
					let _this = this
					let content = _this.replaceEmoji(_this.textMsg)
					_this.$http
					.post('lessonxiangqin/api/lessonfasongxiaoxi', 
					{
						userid : uni.getStorageSync('userId'),
						message: _this.textMsg + '&*&' + _this.user_id + '&*&text'
					}, {})
					.then(res => {
						console.log(res)
						_.each(res.data, o => {
			...
			replaceEmoji(str){
				...
				return '<div style="text-align:left; ">'+replacedStr+'</div>';
				// return replacedStr
			},
